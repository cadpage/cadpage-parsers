...
public class COAdamsCountyBParser extends FieldProgramParser {
  ...
  public COAdamsCountyBParser() {
    super("ADAMS COUNTY", "CO", 
          ...
          "| Address_Changed:MAP! ID89 OLD_NEW_ADDR PLACE CALL CASE2 END " +
          "| ID? MAP ADDR ID8/L ID9/L APT APT PLACE CALL UNIT? " + 
          ") CASE/L? CMD:CH END");
  }
  ...
  private static final Pattern MASTER3 = Pattern.compile("(?:ADDRESS CHANGE)?" + MAP_PTN_STR + " (.*?)(?: ?([A-Z]+\\d+(?:,[A-Z0-9,]*)?))?");
  ...
  protected boolean parseMsg(String subject, String body, Data data) {
    ...
    if (match.matches()) {
      setFieldList("MAP ADDR APT CITY PLACE CALL UNIT");
      ...
      if (data.strPlace.startsWith("AURORA")) {
        data.strCity = "AURORA";
        data.strPlace = data.strPlace.substring(6).trim();
      }
      return true;
    }
  ...
  public Field getField(String name) {
    ...
    if (name.equals("OLD_NEW_ADDR")) return new MyOldNewAddressField();
    ...
    if (name.equals("ID89")) return new MyId89Field();
    if (name.equals("UNIT")) return new MyUnitField();
    if (name.equals("CASE")) return new IdField("Case# *(.*)", true);
    if (name.equals("CASE2")) return new IdField("\\d\\d-[A-Z]{2}-\\d{6}", true);
  ...
  private class MyOldNewAddressField extends AddressField {
    @Override
    public void parse(String field, Data data) {
  ...
  private class MyId89Field extends IdField {
