...
public class UTSaltLakeCountyBParser extends SmartAddressParser {
  ...
  private static final Pattern MASTER_PTN = Pattern.compile("UNIT#:(\\S+) RUN#(\\S+) *, LOC:(.*?)APT#:(.*?)BLDG:(.*), ADD LOC:(.*?)CROSS ST:(.*?), (PAT.\\S*) *PRIORITY:(.*?), INIT.BY:.*?, ZIP:(.*?) LAT/LON(.*?)");
  ...
  protected boolean parseMsg(String body, Data data) {
    ...
    if ((match = MASTER_PTN.matcher(body)).matches()) {
      setFieldList("UNIT ID ADDR CITY APT PLACE INFO X PRI GPS");
      data.strUnit = match.group(1);
