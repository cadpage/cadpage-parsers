...
public class AZYavapaiCountyBParser extends FieldProgramParser {

  public AZYavapaiCountyBParser() {
    super("YAVAPAI COUNTY", "AZ",
          "PN:CALL! ADD:ADDR! XST:X! LAT:GPS1! LON:GPS2 END");
  }
  ...
  private static final Pattern MASTER =
      Pattern.compile("(?:PN: *)?(.*?) (?:Map|MAP:)(?:(\\d+)|NOT FOUND|NOT) +(?:(\\d\\d?/\\d\\d/\\d\\d) +)?(\\d\\d:\\d\\d:\\d\\d) *ADD:(.*?) *([A-Z]+) *XST:(.*)");
