...
public class DispatchA24Parser extends FieldProgramParser {
  ...
  public DispatchA24Parser(String defCity, String defState) {
    super(defCity, defState,
           "UNIT:UNIT? ( CALL:CALL! RUN:SKIP? ( PLACE:PLACE! ADDR:ADDR! BLDG:APT APT:APT CITY:CITY! XSTREETS:X ID:ID% PRI:PRI DATE:DATE% TIME:TIME% CALLERPHONE:PHONE% JURISDICTION:SRC LATITUDE:GPS1/d LONGITUDE:GPS2/d TPPU:SKIP ( ALLUNITS:UNIT |  UNIT:UNIT | ID:ID? ) | ) INFO:INFO " +
                      "| ID:ID! INFO/RN+ " +
                      "| FAIL " +
                      ")");
  }
