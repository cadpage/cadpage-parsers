...
public class NCIredellCountyAParser extends MsgParser {
  
  private static final Pattern MASTER_PTN = Pattern.compile("((?:A|EM|FA|FD|FM|FR|IMA|ISAR)\\d+?(?=[\\.| A-Z]|10-)|MFD|MRS|ICRS|ISAR|SFD|NIRS)[-\\. ]*+([^,]+?), *([^,]*)(?:, *(\\d{2}-\\d{5,6}))?");
  private static final Pattern CODE_CALL_PTN = Pattern.compile("(?:(C-\\d+) |(\\d*)-|([A-Z]+) - )(.*)");
  
  public NCIredellCountyAParser() {
